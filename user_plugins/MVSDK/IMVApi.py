...
if sys.platform == 'win32':
    ...
    if bits == '64bit':
        MVSDKdll = WinDLL("D:/MVviewer/MV Viewer/Application/x64/MVSDKmd.dll")
    else:
        MVSDKdll = WinDLL("D:/MVviewer/MV Viewer/Application/x64/MVSDKmd.dll")
else:
    print("Linux")
...
class MvCamera():
    ...
    def IMV_FlipImage(self, pstFlipImageParam):
        ...
        flipped_image_data = self.get_flipped_image()
        flipped_image = np.frombuffer(flipped_image_data, dtype=np.uint8).reshape(pstFlipImageParam.image_shape)
        pil_flipped_image = Image.fromarray(flipped_image)
        pil_flipped_image.save('flipped_image.jpg')
    # ch:图像顺时针旋转
